"""
MedformerFFT with Bilinear Fusion.
Fused = time_feat^T @ W @ freq_feat + b  (second-order interaction)
"""
import torch
...
class TimeFrequencyModel(nn.Module):
    def __init__(self, configs):
        ...
        # Bilinear fusion: time @ W @ freq
        hidden_dim = 256
        self.W = nn.Bilinear(self.time_feature_dim, configs.d_model, hidden_dim)
        self.fusion_norm = nn.LayerNorm(hidden_dim)
        self.projection = nn.Linear(hidden_dim, configs.num_class)
    ...
    def classification(self, x_enc, x_mark_enc):
        ...
        # Bilinear fusion: captures second-order time-frequency interactions
        fused = self.W(time_feature, freq_feature)  # [B, hidden_dim]
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        raise NotImplementedError("MedformerFFT_bilinear only supports classification.")
